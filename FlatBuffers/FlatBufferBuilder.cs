...
public class FlatBufferBuilder {
    ...
    public FlatBufferBuilder(int initialSize) {
        ...
        _space = initialSize;
        DataBuffer = new ByteBuffer(initialSize);
    }
    ...
    public int Offset => DataBuffer.Length - _space;

    public void Pad(int size) {
        DataBuffer.PutByte(_space -= size, 0, size);
    }

    // Doubles the size of the ByteBuffer, and copies the old data towards
    // the end of the new buffer (since we build the buffer backwards).
    private void GrowBuffer() {
        DataBuffer.GrowFront(DataBuffer.Length << 1);
    }
    ...
    public void Prep(int size, int additionalBytes) {
        ...
        var alignSize =
            (~(DataBuffer.Length - _space + additionalBytes) + 1) &
            (size - 1);
        // Reallocate the buffer if needed.
        while (_space < alignSize + size + additionalBytes) {
            var oldBufSize = DataBuffer.Length;
            GrowBuffer();
            _space += DataBuffer.Length - oldBufSize;
        }
    ...
    public void PutBool(bool x) {
        DataBuffer.PutByte(_space -= sizeof(byte), (byte)(x ? 1 : 0));
    }

    public void PutSbyte(sbyte x) {
        DataBuffer.PutSbyte(_space -= sizeof(sbyte), x);
    }

    public void PutByte(byte x) {
        DataBuffer.PutByte(_space -= sizeof(byte), x);
    }

    public void PutShort(short x) {
        DataBuffer.PutShort(_space -= sizeof(short), x);
    }

    public void PutUshort(ushort x) {
        DataBuffer.PutUshort(_space -= sizeof(ushort), x);
    }

    public void PutInt(int x) {
        DataBuffer.PutInt(_space -= sizeof(int), x);
    }

    public void PutUint(uint x) {
        DataBuffer.PutUint(_space -= sizeof(uint), x);
    }

    public void PutLong(long x) {
        DataBuffer.PutLong(_space -= sizeof(long), x);
    }

    public void PutUlong(ulong x) {
        DataBuffer.PutUlong(_space -= sizeof(ulong), x);
    }

    public void PutFloat(float x) {
        DataBuffer.PutFloat(_space -= sizeof(float), x);
    }
    ...
    public void Put<T>(T[] x)
        where T : struct {
        _space = DataBuffer.Put(_space, x);
    }
    ...
    public void PutDouble(double x) {
        DataBuffer.PutDouble(_space -= sizeof(double), x);
    }
    ...
    ///     and <see cref="ForceDefaults" /> is false, the value will be skipped.
    /// </param>
    ...
    public StringOffset CreateString(string s) {
        ...
        DataBuffer.PutStringUTF8(_space -= utf8StringLen, s);
    ...
    public int EndTable() {
        ...
        for (i = 0; i < _numVtables; i++) {
            var vt1 = DataBuffer.Length - _vtables[i];
            var vt2 = _space;
            var len = DataBuffer.GetShort(vt1);
            if (len == DataBuffer.GetShort(vt2)) {
                for (var j = sizeof(short); j < len; j += sizeof(short)) {
                    if (DataBuffer.GetShort(vt1 + j) != DataBuffer.GetShort(vt2 + j)) {
                        goto endLoop;
                    }
                }
        ...
        if (existingVtable != 0) {
            ...
            _space = DataBuffer.Length - vtableloc;
            // Point table to existing vtable.
            DataBuffer.PutInt(_space, existingVtable - vtableloc);
        }
        else {
            // No match:
            ...
            DataBuffer.PutInt(DataBuffer.Length - vtableloc, Offset - vtableloc);
        }
    ...
    public void Required(int table, int field) {
        var table_start = DataBuffer.Length - table;
        var vtable_start = table_start - DataBuffer.GetInt(table_start);
        var ok = DataBuffer.GetShort(vtable_start + field) != 0;
    ...
    protected void Finish(int rootTable, bool sizePrefix) {
        ...
        AddOffset(rootTable);
        if (sizePrefix) {
            AddInt(DataBuffer.Length - _space);
        }

        DataBuffer.Position = _space;
    }
    ...
    public byte[] SizedByteArray() {
        return DataBuffer.ToSizedArray();
    }
