...
public class UserPackageSelectionStore : IDisposable {
    private readonly BehaviorSubject<PackageState> _state = new BehaviorSubject<PackageState>(new PackageState());
    ...
    public IObservable<PackageState> Observe() {
        return _state.AsObservable();
    }

    public IObservable<Dictionary<PackageKey, PackageAction>> SelectedPackages() {
        return Observe().Map(state => state.SelectedPackages);
    }
